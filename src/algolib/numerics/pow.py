...
Number = Union[float, complex, "Complex"]
...
def pow(base: Number, exp: Any) -> Number:
    ...
    provided for API consistency within algolib. For built-in numeric type (int, float),
    ...
    Parameters
    ----------
    base : int or float or Complex
        The base value.
    exp : Any
        The exponent. For built-in numeric types this follows Python semantics.
        For `Complex`, only integer exponents are guaranteed to be supported.
    ...
    - For `Complex`, non-integer exponents return `NotImplemented` from `__pow__`,
