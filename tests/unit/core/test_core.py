...
@pytest.mark.parametrize(
    "bad", [("1", 2), (1, "2"), (None, 0), (object(), 0), (1, object())]
)
...
@pytest.mark.parametrize("bad_iter", [(1,), (1, 2, 3), "12", [1, "x"], object()])
def test_from_iterable_errors(bad_iter):
    if (
        isinstance(bad_iter, (tuple, list))
        and len(bad_iter) == 2
        and any(not isinstance(x, (int, float)) for x in bad_iter)
    ):
        # 解包成功，但内部构造会因类型报错
        with pytest.raises(InvalidTypeError):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            Complex(3, 4),
            Complex(
                (1 * 3 + 2 * 4) / (3 * 3 + 4 * 4), (2 * 3 - 1 * 4) / (3 * 3 + 4 * 4)
            ),
        ),
